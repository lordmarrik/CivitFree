// Fullscreen model picker — image-card layout with preview, badges, version + Select.
// Tabs: Local Models / Browse CivitAI. Category chips + sort/filter row.

function PickerCard({ name, type, base, version, versions, size, palette, seed, onSelect, loaded }) {
  const p = window.PALETTES ? window.PALETTES[(palette || 0) % 8] : ['#1d2240','#3a2563','#7d3a8a','#d65a8a'];
  const s = seed || 1234;

  // Generate a 2x3 or single preview grid
  const isGrid = (s % 3) !== 0;
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* Badges */}
        <div style={{
          position:'absolute', top: 8, left: 8, display:'flex', gap: 4, zIndex: 2,
        }}>
          <span style={{
...
// ─── Model Picker ─────────────────────────────────────
function ModelPicker({ open, onClose }) {
...
// ─── LoRA Picker ──────────────────────────────────────
function LoraPicker({ open, onClose }) {
  const [sourceTab, setSourceTab] = React.useState('local');
  const [browseTab, setBrowseTab] = React.useState('ALL');
  const [localTab, setLocalTab] = React.useState('ALL');
  const [category, setCategory] = React.useState(null);
...
Object.assign(window, { ModelPicker, LoraPicker, PickerCard, PickerTabs, PickerCategories, PickerSortRow });
