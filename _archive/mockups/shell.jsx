// Shell components — chrome wrappers used by every variant.
// CFFrame      — outer 390x780 device-shaped frame
// StatusBar    — fake iOS status bar (mono numerals to feel technical)
// TopBar       — header w/ logo, nav icons, settings toggle
// Dock         — sticky generate footer with queue/breakdown row
// SectionTitle — labelled section header

function StatusBar({ time = '14:22' }) {
  return (
...
function TopBar({ active = 'brush', settings = false, onTab, personal = false, onMenu, onBookmark }) {
  const tab = (key, icon) => (
...
function Dock({ qty = 2, price = 5, label = 'Generate', personal = false, etaSec, gpu, onGpuClick }) {
  return (
    <div className="cf-dock">
      ...
      {/* QTY — full-width row above Generate */}
      {personal && (
        <div style={{display:'flex', alignItems:'center', gap: 10, padding:'0 0 8px', marginBottom: 2}}>
          ...
          <div style={{
            ...
            <button style={{width:24, height:24, display:'flex', alignItems:'center', justifyContent:'center', color:'var(--text-dim)'}}>
              <Ic.Minus size={14}/>
            </button>
            <span>{qty}</span>
            <button style={{width:24, height:24, display:'flex', alignItems:'center', justifyContent:'center', color:'var(--text-dim)'}}>
...
function SectionTitle({ children, required, action }) {
  return (
...
function CFFrame({ children, label, sublabel }) {
  return (
...
Object.assign(window, { CFFrame, StatusBar, TopBar, Dock, SectionTitle });
