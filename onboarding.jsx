// Onboarding flow — 2 screens shown as a fullscreen overlay.
// Screen 1: Enter ComfyUI backend URL + Test Connection
// Screen 2: Scan results + pick default model
...
// Standalone onboarding frames for the design canvas artboards
function OnboardingFrame({ step }) {
  ...
  return (
    <div className="cf-frame">
      ...
      {step === 1 && (
        <div style={{flex:1, padding:'0 18px', display:'flex', flexDirection:'column'}}>
          ...
          <div style={{padding:'16px 0 24px', display:'flex', gap: 8}}>
            ...
            <button style={{
              flex:1, padding:'13px', borderRadius: 10,
              background:'var(--accent)', fontWeight: 600, fontSize: 14, color:'white',
            }}>Continue</button>
          </div>
        </div>
      )}
      ...
      {step === 2 && (
        <div style={{flex:1, padding:'0 18px', display:'flex', flexDirection:'column', overflow:'auto'}}>
          <div style={{fontSize: 16, fontWeight: 600, marginBottom: 6}}>Resources Detected</div>
          <div className="dim" style={{fontSize: 13, marginBottom: 16, lineHeight: 1.5}}>
            We scanned your ComfyUI installation and found:
          </div>
          <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap: 8, marginBottom: 20}}>
            {[{n:'12',label:'Checkpoints'},{n:'47',label:'LoRAs'},{n:'3',label:'VAEs'},{n:'8',label:'Embeddings'}].map(s => (
              <div key={s.label} style={{
                background:'var(--panel)', border:'1px solid var(--line)', borderRadius: 10,
                padding:'14px 12px', textAlign:'center',
              }}>
...
Object.assign(window, { OnboardingFlow, OnboardingFrame });
