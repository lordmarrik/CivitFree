...
| Heart | Favorite/save image locally. | Future local metadata, unless Civitai account sync is intentionally added later. |
| Wand | Opens image action sheet / transform actions. | Real branch point. |
| Download icon | Saves image to device/PC. | Future; requires direct `/view` download or local cache strategy. |
...
| Download | Save selected output. | Should be implemented before destructive/delete flows. |
...
| Mockup element | Supposed to be | CivitFree behavior |
| --- | --- | --- |
| Sort newest/oldest | Changes Queue/Feed ordering. | Wire directly to run/tile sorting. |
| Favorited only | Shows locally favorited images/runs. | Requires favorite persistence. |
